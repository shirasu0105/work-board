/* ============================================================
   icons.jsx — シンプルな線アイコン + ステータス円 + ブランドマーク
   （UIアイコンのみ。複雑なイラストは描かない）
   ============================================================ */

function Icon(props) {
  var name = props.name, size = props.size || 16, cls = props.className || '';
  var s = { width: size, height: size, display: 'block', flex: 'none' };
  var common = {
    viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor',
    strokeWidth: props.weight || 1.8, strokeLinecap: 'round', strokeLinejoin: 'round',
    style: s, className: cls
  };
  var P = {
...
function StatusIcon(props) {
  var status = props.status, size = props.size || 16;
  var meta = window.Store.statusMeta[status] || {};
  var c = meta.color;
  var common = { width: size, height: size, viewBox: '0 0 16 16', style: { display: 'block', flex: 'none' } };
  if (status === 'done') {
    return (
      <svg {...common}>
